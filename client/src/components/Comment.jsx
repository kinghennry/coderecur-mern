...
export default function Comment({ commentItem, onLike, onDelete }) {
  ...
  const checkUserId = currentUser?.id || currentUser?._id
  return (
        ...
        {isEditing ? (
          ...
          <>
            ...
            <div className='flex items-center pt-2 text-xs border-t dark:border-gray-700 max-w-fit gap-2'>
              <button
                type='button'
                onClick={() => onLike(commentItem._id || currentUser?.id)}
                className={`text-gray-400 hover:text-blue-500 
                  ${
                    currentUser &&
                    commentItem.likes.includes(
              ...
              {currentUser &&
                (checkUserId === commentItem.userId || currentUser.isAdmin) && (
                  <>
                    <button
                      type='button'
                      onClick={handleEdit}
